---
title: "IGV: programmatic access to the web browser-based genome track viewer, igv.js"
author: "Paul Shannon"
package: IGV
date: "`r Sys.Date()`"
output:
...
vignette: >
  %\VignetteIndexEntry{"IGV: programmatic access to igv.js, a browser-based genome track viewer"}
...
The IGV package provides easy programmit access in R to the web-based javascript library
...
We introduce IGV via a case study of genetic variants associated with Alzheimer's Disease as
described in the 2014 Nature Genetics paper "Meta-analysis of 74,046 individuals identifies 11 new
susceptibility loci for Alzheimer’s
disease"^[Lambert, Jean-Charles, et al. "Meta-analysis of 74,046 individuals identifies 11 new susceptibility loci for Alzheimer's disease." Nature genetics 45.12 (2013): 1452.]
We focus on a 1 megabase region surrounding the MEF2C gene, which is implicated in Alzheimer's
Disease.  This megabase region contains 208 variants whose presence is associated with the
...
We will proceed as follows:
 ...
 - create our first track - the 208 SNPs - using the most primitive approach: using a base R data.frame

 - employ Bioconductor's AnnotationHub to retrieve three methylation datasets; these are associated
    with open chromatin and transcription factor binding sites.
    With each of these datasets we create an IGV track (of an approriate type - IGV's Track class
    has a variety of subclasses depending on data type and data source location)

 - for good measure we conclude by loading a very small slice of a different variant dataset, in VCF format, from another
   Alzheimer's [study](http://adni.loni.usc.edu/data-samples/genetic-data/wgs/) of 818
   participants.  VCF files typically contain fine-grained variant data, as is seen in the igv.js
   popup dialog.
...
In order to illustrate some of genomic data exploration capabilities of IGV we are therefore mixing
