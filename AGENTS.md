...
5. **Front-end / JavaScript**: 
   The JavaScript code is located in `inst/browserCode/`. If modifications are made to the JavaScript source, the bundle must be rebuilt. You can do this by running `make browserCode` from the package root.
6. **General Make Targets**:
   The `makefile` provides useful targets: `make roxy` (document), `make build`, `make vig` (build vignettes), and `make quick` (browserCode, roxy, install).
